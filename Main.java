...
public final class Main {
    ...
    public static void main(String[] args) {
       ...
       Scanner sc=new Scanner(System.in);
        String str=null;
        System.out.print("写个string:");
        str=sc.nextLine();       //读取用户输入

        String theme = str ;

        //判断是不是读书笔记
        if(theme.contains("《")){
            String res = BookFEIHUA(theme,6000);
            System.out.println(res); //输出废话

        }

        else {
            String res = FEIHUA(theme );
            System.out.println(res); //输出废话
    ...
    //主代码
    private static String FEIHUA(String theme)
    {
        return FEIHUA(theme,6000);
    }

    private static String BookFEIHAU(String theme)
    {
        return BookFEIHUA(theme,6000);
    }


    private static String BookFEIHUA(String theme, int sum)
    {
        StringBuffer res = new StringBuffer(sum);
            res.append("通过阅读"+theme+"这本书,使我受益匪浅,让我懵懂的心灵渐渐领悟了许多重要的的道理.我想这正是这本书对我而言的重要意义."+"\r\n");
            res.append("这本书里的每个主人公都是那么的善良、正直、乐于助人、团结友爱，每当我捧起这本书，" +
                "津津有味的看时，就觉得书里面的一切人物都在我的身旁，仿佛身临其境。");
        while(res.length()<sum){
            ...
            if (num < 25){
                ...
                //25%概率说个名言
                res.append(randomChoice(bfamous)
                        .replace("a",brandomChoice(before))
                        .replace("b",brandomChoice(after))
                );
                ...
                res.append(randomChoice(duanzi)
                        .replace("A",theme)
                );
                if((int) (Math.random() * 150)<60){
                    //写完段子后30%概率换个行
                    res.append("\r\n");
                }
            }
                ...
                //70%概率说着废话，为了不能跑题啊
                res.append(randomChoice(bremark)
                        .replace("x",theme)
                );
            }
        ...
        res.append("\r\n");
        res.append("当我读完"+theme+"后"+"我们要学会生活，体验生活，抱着一种享受人生的态度去学习，去工作，去休息。" +
                "我们的成长更多来自于心智的成熟，这种成长的过程中，我们会面临更多选择，" +
                "懂得思考，懂得选择合适的道路，懂得如何行动，懂得品位人生，才不枉美好时光。");

        return res.toString();
    }

    public static <String> String brandomChoice(List<String> data) {
    ...
    static List<String> duanzi = Arrays.asList("书是一盏铮亮的灯，照亮我心中的黑暗，让我明白了世间的善恶美丑；" +
            "有时候，书是一对洁白的翅膀，带我飞到云海深处，一览山河的壮丽美景；" +
            "有时候，书是一叶坚固的小舟，带我驶向成功的彼岸，任凭风吹浪打也不回头。",
            "书中精彩的情节往往让我忘记了一切，整个身心都沉浸在书的世界里。" +

                    "当A中的主人公胜利时，我会高兴的手舞足蹈；" +
                    "当邪恶没有受到惩罚时，我会气得直跺脚；" +
                    "当英雄牺牲时，我的眼泪会顺着脸颊“吧嗒吧嗒”往下流。",

            " 为什么我的生活变得这么充实？为什么我感到生命是如此的精彩？" +
                    "因为“书中自有黄金屋，书中自有颜如玉”。" +
                    "A是无穷的宝藏，为我增添了丰富的知识；" +
                    "A是快乐的天堂，让我忘记了所有的忧伤。" +
                    "如果没有A，我的生活将是多么的寂寥空虚，我的生命将是多么的暗淡无光。",
            ...
            "读书可供消遣，可供装饰，也可供增长才干。" +
                    "为消遣而读书，常见于独处退居之时；" +
                    "为装饰而读书，多用于高谈阔论之中；" +
                    "为增长才干而读书，主要在于对事物的判断和处理。",

            "正确的读书方法，也许应该是，在思想上要重视读书，要在有选择的前堤下读好书，时间上对读书要给予保证，" +
                    "但我们也要勇敢地走出书本，勇于在实践中，去探索、去发现，去解决实际的问题，" +
                    "只有这样，我们读书的价值才能得到真正的体现，我们的读书行为才能真正得到社会的认可。",

           " A，是一位知识渊博的老师，它带我畅游理性世界，领略大自然风光，了解大自然奥秘，" +
                   "能让我懂得许多人生哲理。A，用它丰富的知识甘露，浇灌了我求知的心田。",

            "每读一次A，能加深我们的理解能力，让灵魂感到冲突。" +
                    "“读书百遍，其义自现”。每次读A，都能秒趣横生，并从中得到可贵的知识。",

            "真正的读书源自内心的热爱，这份爱纯粹天然，就像向阳花总是追寻太阳，" +
                    "不为名利，听从的是心的召唤，正是这份爱让人无论何时何地何种境遇都能嗅出书的馨香，" +
                    "都能在孤独中以温情的手指在书的页面上摩挲不止。在主动碰撞的激情中，" +
                    "A中的人与事，作者的理念智慧与我们的经历体会、思想情感互动沟通，相鸣相和。" +
                    "日子长了，我们便获得了对生命对人生最深切地认知与感悟。 "
    );

    static List<String> bfamous = Arrays.asList(
           "古人a，读书破万卷，下笔如有神。b",
            " 古人a：书犹药也，善读之可以医愚。b",
            ...
            "高士其a，书籍有如人体血液一样宝贵。人缺少了血液，身体就要衰弱，人缺少了知识，头脑就要枯竭。b",
    ...
    static List<String> bremark = Arrays.asList(
            "现在, 解决x中所提到的问题, 是非常非常重要的. 所以, ",
            "我们不得不面对一个非常尴尬的事实, 那就是, ",
            "x中的事情发生, 到底需要如何做到, 即使x中的悲剧不发生, 又会如何产生. ",
            "而这些并不是完全重要, 更加重要的问题是, ",
            "x中主人公的理想, 到底应该如何实现. ",
            "带着这些问题, 我们来审视一下x的时代背景. ",
            "所谓x的成功之处, 关键是x中生动详细的细节需要如何写. ",
            "我们一般认为, 抓住了问题的关键, 其他一切则会迎刃而解.",
            "问题的关键究竟为何? ",
            "x中的悲剧因何而发生?",
            "每个人都不得不面对这些问题.  在面对这种问题时, ",
            "一般来讲, 我们都必须务必慎重的考虑考虑. ",
            "要想清楚, x中的人物和时代, 到底是一种怎么样的存在. ",
            "了解清楚x的作者到底是一种怎么样的存在, 是解决一切问题的关键.",
            "就我个人来说, x中的道理对我的意义, 不能不说非常重大. ",
            "本人也是经过了深思熟虑,在每个日日夜夜思考这个问题. ",
            "x所描述的世界, 发生了会如何, 不发生又会如何. ",
            "在这种困难的抉择下, 本人思来想去, 寝食难安.",
            "生活中, 若x的愿出现了, 我们就不得不考虑它出现了的事实. ",
            ...
            "x的出现似乎是一种巧合，但如果我们从一个更大的角度看待问题，这似乎是一种不可避免的事实. ",
            "在这种不可避免的冲突下，我们必须解决这个问题. ",
            "对我个人而言，读x这本书不仅仅是一个优秀的选择，还可能会改变我的人生. "
    );


   //通用废话
    private static String FEIHUA(String theme, int sum)
    {
        StringBuffer res = new StringBuffer(sum);
        while(res.length()<sum){
    ...
    public static <String> String randomChoice(List<String> data) {
        int num = (int) (Math.random() * data.size());
        return data.get(num);
    }
                                         //随机取一个工具函数
